...
if not os.path.exists('./record-time-baseline' + '/plots/'):
    os.makedirs('./record-time-baseline' + '/plots/')
if not os.path.exists('./record-time-baseline' + '/model/'):
    os.makedirs('./record-time-baseline' + '/model/')

path = './record-time-baseline' + '/plots/'
path_best_model = './record-time-baseline' + '/model/best_model.pth'
...
in_pt=1000
out_pt=100
x_inner = np.random.uniform(0, 0.02, in_pt)
x_outer = np.random.uniform(0.03, 1, out_pt)
x_all=np.hstack((x_inner,x_outer))
x_all_reshape=x_all.reshape(-1,1)
x_all_reshape_train = tensor(x_all_reshape[:ntrain, :])
x_all_reshape_test = tensor(x_all_reshape[-ntest:, :])

train_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset
                                           (ab_train, x_all_reshape_train),
                                           batch_size=batchsize,
                                           shuffle=True)

test_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset
                                          (ab_test, x_all_reshape_test),
                                          batch_size=batchsize,
                                          shuffle=False)
...
for ep in range(epochs):
    ...
    train_loss_eqn = 0
    train_loss_bc=0
    for ab, x_all_train in train_loader:
        net.zero_grad()
        Loss_eqn = loss_eqn(ab, x_all_train)
    ...
    for ab, x_all_test in test_loader:
        test_eqn = loss_eqn(ab, x_all_test)
    ...
    if test_mse < best_loss:
        ...
        with open('./record-time-baseline' + '/model/best epoch', 'w') as f:
            f.write('best epoch: ')
            f.write(str(ep + 1))
    if ep % 10000 == 0:
...
with open('./record-time-baseline/test error', 'w') as f:
    f.write('test_rel_l2: ')
...
for i in range(ntest):
    if i % 25 == 0:
        ax.plot(x_all, y_numerical_sol[i, :], 'b-', label='Numerical solution', linewidth=2, zorder=0)
        ax.plot(x_all, y[i, :].cpu().detach().numpy(), 'r--', label='deeponet', linewidth=2,
                zorder=1)
ax.legend(['Numerical solution', 'deeponet'], loc='best')
...
for i in range(ntest):
    if i % 25 == 0:
        axins.plot(x_all, y_numerical_sol[i, :], 'b-', label='Numerical solution', linewidth=2, zorder=0)
        axins.plot(x_all, y[i, :].cpu().detach().numpy(), 'r--', label='deeponet',
