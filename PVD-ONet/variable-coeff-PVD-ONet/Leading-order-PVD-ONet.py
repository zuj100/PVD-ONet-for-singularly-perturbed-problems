...
if not os.path.exists('./record-time-0' + '/plots/'):
    os.makedirs('./record-time-0' + '/plots/')
if not os.path.exists('./record-time-0' + '/model/'):
    os.makedirs('./record-time-0' + '/model/')

path = './record-time-0' + '/plots/'
path_best_inner_model = './record-time-0' + '/model/best_inner_model.pth'
path_best_outer_model = './record-time-0' + '/model/best_outer_model.pth'
...
net_outer = deeponet([mm, net_width, net_width, net_width, net_width, net_width],
                     [1, net_width, net_width, net_width, net_width, net_width]).to(device)
...
def loss_inner(ab, x):
    ...
    ab = ab.view(-1, 2)
    x = x.to(device)
    a_0 = torch.tensor(a_coeff(xb_0)).to(device)
    x.requires_grad = True
    net_in = x
...
for ep in range(epochs):
    ...
    if test_mse < best_loss:
        ...
        with open('./record-time-0' + '/model/best epoch', 'w') as f:
            f.write('best epoch: ')
            f.write(str(ep + 1))
    if ep % 10000 == 0:
...
with open('record-time-0/test error', 'w') as f:
    f.write('test_rel_l2: ')
...
for i in range(ntest):
    if i % 25 == 0:
        ...
        ax.plot(x_all, y[i, :].cpu().detach().numpy(), 'r--', label='Leading-order PVD-ONet', alpha=1,linewidth=2)
