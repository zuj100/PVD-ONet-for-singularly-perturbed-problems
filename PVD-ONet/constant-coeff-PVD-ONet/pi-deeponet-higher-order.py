...
if not os.path.exists('./pi-deeponet-higher-order-5-40-silu'+ '/plots/'):
    os.makedirs('./pi-deeponet-higher-order-5-40-silu'  + '/plots/')
if not os.path.exists('./pi-deeponet-higher-order-5-40-silu'+ '/model/'):
    os.makedirs('./pi-deeponet-higher-order-5-40-silu'  + '/model/')

path='./pi-deeponet-higher-order-5-40-silu' + '/plots/'
path_best_inner_a_model = './pi-deeponet-higher-order-5-40-silu' + '/model/best_inner_a_model.pth'
path_best_inner_b_model = './pi-deeponet-higher-order-5-40-silu' + '/model//best_inner_b_model.pth'
path_best_inner_c_model = './pi-deeponet-higher-order-5-40-silu' + '/model/best_inner_c_model.pth'
path_best_outer_a_model = './pi-deeponet-higher-order-5-40-silu' + '/model/best_outer_a_model.pth'
path_best_outer_c_model = './pi-deeponet-higher-order-5-40-silu' + '/model/best_outer_c_model.pth'
...
#         with open('./pi-deeponet-higher-order-5-40-silu' + '/model/best epoch', 'w') as f:
#             f.write('best epoch: ')
#             f.write(str(ep + 1))
#     if ep % 10000 == 0:
...
extrema_pt_reshape=tensor(extrema_pt_value[:,0].reshape(-1,1)).cuda() #[100,1]




ab_test = ab_test.cuda()
y_outer_extrema_pt=net_outer_a(ab_test,extrema_pt_reshape)+ eps *net_outer_c(ab_test,extrema_pt_reshape) #100
y_inner_extrema_pt=net_inner_a(ab_test,extrema_pt_reshape/eps)+eps*(net_inner_c(ab_test,extrema_pt_reshape/eps)
                                                            +net_inner_b(ab_test,extrema_pt_reshape/eps)
                                                                    *(extrema_pt_reshape.flatten()/eps)) #100
betae=torch.vstack(list(map(lambda ab,x:minus_term(ab,xb_0,x), ab_test,extrema_pt_value[:,0].reshape(-1,1)))) #100,1
...
y_inner1 = torch.vstack(list(map(lambda ab:net_inner_a(torch.tile(ab.view(-1,2),(in_pt,1)),xi_inner_reshape),
                                 ab_test)))\
           +eps * (torch.vstack(list(map(lambda ab:net_inner_c(torch.tile(ab.view(-1,2),(in_pt,1)),xi_inner_reshape),ab_test)))
                   +torch.vstack(list(map(lambda ab:net_inner_b(torch.tile(ab.view(-1,2),(in_pt,1)),xi_inner_reshape),ab_test)))
                  * xi_in)
...
with open('pi-deeponet-higher-order-5-40-silu/test error', 'w') as f:
    f.write('test_rel_l2: ')
...
for i in range(ntest):
    if i % 25 == 0:
        ...
        ax.plot(x_all,y[i,:].cpu().detach().numpy(),'r--',label='High-order PVD-ONet',linewidth=2, alpha=1.0, zorder=1)
ax.legend(['Analytical solution', 'High-order PVD-ONet'], loc='best')
ax.set_xlim(0, 1)
ax.set_ylim(0, 7)
ax.margins(0)
