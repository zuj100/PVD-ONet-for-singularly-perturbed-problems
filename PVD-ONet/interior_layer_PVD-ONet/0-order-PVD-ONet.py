...
if not os.path.exists('./0-order-PVD-ONet'+ '/plots/'):
    os.makedirs('./0-order-PVD-ONet'  + '/plots/')
if not os.path.exists('./0-order-PVD-ONet'+ '/model/'):
    os.makedirs('./0-order-PVD-ONet'  + '/model/')

path='./0-order-PVD-ONet' + '/plots/'
path_best_inner_model = './0-order-PVD-ONet' + '/model/best_inner_model.pth'
path_best_outer_left_model = './0-order-PVD-ONet' + '/model/best_outer_left_model.pth'
path_best_outer_right_model = './0-order-PVD-ONet' + '/model/best_outer_right_model.pth'
...
net_inner = deeponet([mm,net_width,net_width,net_width,net_width,net_width],
                     [1,net_width,net_width,net_width,net_width,net_width]).to(device)

net_outer_left = deeponet([mm,net_width,net_width,net_width,net_width,net_width],
                          [1,net_width,net_width,net_width,net_width,net_width]).to(device)

net_outer_right = deeponet([mm,net_width,net_width,net_width,net_width,net_width],
                           [1,net_width,net_width,net_width,net_width,net_width]).to(device)

total_params = (
    sum(p.numel() for p in net_inner.parameters())
    + sum(p.numel() for p in net_outer_left.parameters())+
+ sum(p.numel() for p in net_outer_right.parameters()))
...
optimizer = optim.Adam(list(net_inner.parameters())+ list(net_outer_left.parameters())
                       + list(net_outer_right.parameters()),
...
def loss_outer_left(ab,x):
    ab = ab.cuda()
    ab = ab.view(-1, 2)
    x = x.to(device)
    x.requires_grad = True
    net_in = x
    y = net_outer_left(ab,net_in)
    y = y.reshape(-1,1)
...
def loss_outer_right(ab,x):
    ab = ab.cuda()
    ab = ab.view(-1, 2)
    x = x.to(device)
    x.requires_grad = True
    net_in = x
    y = net_outer_right(ab,net_in)
    y = y.reshape(-1,1)
    y_x = torch.autograd.grad(y, x, grad_outputs=torch.ones_like(y), create_graph=True, only_inputs=True)[0]
    loss_1 = y-y*y_x
    loss_f = nn.MSELoss()
...
def loss_bc(ab,x_0,x_1):
    ...
    y_out_left  = net_outer_left(ab,x_0).reshape(-1,1)
    y_out_right = net_outer_right(ab,x_1).reshape(-1,1)

    loss_f = nn.MSELoss()
    loss = loss_f(y_out_right,ab[:,1].reshape(-1,1))+loss_f(y_out_left,ab[:,0].reshape(-1,1))
    return loss

def loss_BC_match(ab,xb, yb_plus,yb_minus):
    ab = ab.cuda()
    ab = ab.view(-1, 2)
    ...
    y_left = net_outer_left(ab,x).reshape(-1,1)
    y_right = net_outer_right(ab, x).reshape(-1, 1)

    y_inner_plus = net_inner(ab, yb_plus).reshape(-1,1)
    y_inner_minus = net_inner(ab, yb_minus).reshape(-1, 1)

    loss_f = nn.MSELoss()
    loss = loss_f(y_inner_minus,y_left)+loss_f(y_inner_plus,y_right)
    return loss
...
#     net_inner.train()
#     net_outer_left.train()
#     net_outer_right.train()
#     train_mse = 0
#     train_loss_outer = 0
#     train_loss_inner = 0
#     train_loss_bc = 0
...
#         optimizer.zero_grad()
#
#         Loss_outer = loss_outer_left(ab,x_out_train)+loss_outer_right(ab,x_out_train)
#         Loss_inner = loss_inner(ab,xi_train)
#         Loss_bc = loss_bc(ab,xb_0,xb_1)
#         Loss_match = loss_BC_match(ab,x_bd,xb_20,-xb_20)
...
#         test_outer = loss_outer_left(ab, x_out_test)+loss_outer_right(ab, x_out_test)
#         test_inner = loss_inner(ab, xi_test)
#         test_bc = loss_bc(ab, xb_0, xb_1)
#         test_match = loss_BC_match(ab,x_bd,xb_20,-xb_20)
...
z_inner_reshape = torch.tensor(z_inner.reshape(-1, 1),dtype=torch.float32).to(device)
...
x_left_inner_reshape=torch.tensor(x_left_inner.reshape(-1, 1), dtype=torch.float32).to(device)
x_right_inner_reshape=torch.tensor(x_right_inner.reshape(-1, 1), dtype=torch.float32).to(device)
...
y_inner = torch.vstack(list(map(lambda ab:net_inner(torch.tile(ab.view(-1,2),(in_pt,1)),z_inner_reshape),
                                 ab_test))) #(100,10000)

y_outer_left = torch.vstack(list(map(lambda ab:net_outer_left(torch.tile(ab.view(-1,2),(5100,1)),x_left_inner_reshape),
                                 ab_test))) #(100,5100)
y_outer_right = torch.vstack(list(map(lambda ab:net_outer_right(torch.tile(ab.view(-1,2),(5100,1)),x_right_inner_reshape),
                                 ab_test))) #(100,5100)

x_bd = tensor(x_bd).to(device)
left=torch.hstack(list(map(lambda ab:net_outer_left(torch.tile(ab.view(-1,2),(1,1)),x_bd),
                                 ab_test))) #100

left=torch.tile(left[:,None],(1,100))#(100,100)

right=torch.hstack(list(map(lambda ab:net_outer_right(torch.tile(ab.view(-1,2),(1,1)),x_bd),
                                 ab_test))) #100
right=torch.tile(right[:,None],(1,100))#(100,100)



y_inner_left = np.hstack((left.cpu().detach().numpy(), y_inner[:,:5000].cpu().detach().numpy())) #(100,5100)
y_inner_right = np.hstack((y_inner[:,-5000:].cpu().detach().numpy(),right.cpu().detach().numpy())) #(100,5100)

y_left = y_inner_left + y_outer_left.cpu().detach().numpy() - left[:,0:1].cpu().detach().numpy()
y_right = y_inner_right + y_outer_right.cpu().detach().numpy() - right[:,0:1].cpu().detach().numpy()

y=np.hstack((y_left,y_right)) #(100,10200)
...
with open('./0-order-PVD-ONet/test error', 'a') as f:
    f.write('test_rel_l2: ')
...
for i in range(ntest):
    if i % 31== 0:
        ...
        ax.plot(x_all,y[i,:],'r--',label='Leading-order PVD-ONet',linewidth=3, alpha=1.0, zorder=1)
