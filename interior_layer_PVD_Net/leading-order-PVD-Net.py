...
for lp in range(loop):
    ...
    path_best_outer_left_model='./leading-order-PVD-Net-new-sample'+'/model/loop_{:.0f}/best_outer_left_model.pth'.format(lp + 1)
    path_best_outer_right_model = './leading-order-PVD-Net-new-sample' + '/model/loop_{:.0f}/best_outer_right_model.pth'.format(lp + 1)
    ...
    ############################################################
    optimizer=optim.Adam(list(net_inner.parameters())+list(net_outer_left.parameters())+list(net_outer_right.parameters()),
    ...
    def eqn_inner(x):
        ...
        tmp = y_inner*y_inner_x-y_inner_xx
        criterion = nn.MSELoss()
        return criterion(tmp, torch.zeros_like(tmp))
    ...
    for epoch in range(epochs):
        ...
        net_inner.zero_grad()
        net_outer_left.zero_grad()
        net_outer_right.zero_grad()
        ...
        loss = (loss_eqn_outer_left + loss_eqn_inner
                + loss_bc_outer_left + loss_bc_match+ loss_eqn_outer_right + loss_bc_outer_right)# +
        loss.backward()
        ...
        if epoch % 1000 == 0:
            ...
            print('Loss_eqn-outer_right: {:.10f}  Loss_bc_outer_left: {:.8f}   Loss_bc_outer_right: {:.8f} Loss_bc_match: {:.8f}  '
                  .format(loss_eqn_outer_right.item(), loss_bc_outer_left.item(), loss_bc_outer_right.item(),loss_bc_match.item()))
            print('-----------------------------------------------------------------------------------')
    toc = time.time()
    elapseTime = toc - tic
    ...
    ax.plot(x_all, y ,'r--', label='Leading-order approximate solution', alpha=1.,linewidth=3)  # PINN
    ...
    ax.margins(0)
    bbox = [0, 0, 1, 1]
    axins = inset_axes(ax, width='40%', height='30%', loc="center right", bbox_to_anchor=bbox, bbox_transform=ax.transAxes)
    axins.plot(x_all, y_numerical, 'b-', label='Numerical solution', alpha=1.0,linewidth=3)  # Numerical
    axins.plot(x_all, y, 'r--', label='Leading-order approximate solution', alpha=1.,linewidth=3)  # PINN
