...
if not os.path.exists('./0-order-new-sample' + '/plots/'):
    os.makedirs('./0-order-new-sample' + '/plots/')
path = './0-order-new-sample' + '/plots/'
# CUDA support
...
for lp in range(loop):
    if not os.path.exists('./0-order-new-sample' + '/model/loop_{:.0f}/'.format(lp + 1)):
        os.makedirs('./0-order-new-sample' + '/model/loop_{:.0f}/'.format(lp + 1))
    path_best_inner_model='./0-order-new-sample'+'/model/loop_{:.0f}/best_inner_model.pth'.format(lp + 1)
    path_best_outer_model='./0-order-new-sample'+'/model/loop_{:.0f}/best_outer_model.pth'.format(lp + 1)
    ...
    for epoch in range(epochs):
        ...
        if loss<best_loss:
            ...
            with open('./0-order-new-sample' + '/model/loop_{:.0f}/best epoch'.format(lp + 1), 'w') as f:
                f.write('best epoch: ')
                f.write(str(epoch+1))
    ...
    np.savez('../Truncation_parameter/A=20/results_A_20.npz',
             x_all=x_all,
             y_analytical=y_analytical,
             y_pred=y)
    with open('0-order-new-sample/test error', 'a') as f:
        f.write('\nloop_{:.0f}\n'.format(lp + 1))
        f.write('test_rel_l2: ')
    ...
    axins = inset_axes(ax, width='40%', height='60%', loc="lower center", bbox_to_anchor=(0.005, 0.1, 0.6, 1),
